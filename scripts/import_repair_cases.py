...
"""
批量导入维修案例
从 JSON 文件读取维修案例数据，导入到 SQLite 和 Qdrant
"""
...
def save_case_to_db(case_data: dict) -> int:
    ...
    cursor = conn.cursor()
    cursor.execute('''
        INSERT INTO cases (case_type, title, device_type, fault_symptom, fault_cause, solution, parts_used, technician, notes, case_text)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', (
        "repair",
        case_data.get("title", ""),
...
def sync_to_qdrant(case_id: int, case_data: dict):
    ...
    try:
        ...
        # 构建可检索的文本
        search_text = f"{case_data['title']} | 设备: {case_data.get('device_type', '')} | 故障: {case_data.get('fault_symptom', '')} | 原因: {case_data.get('fault_cause', '')} | 方案: {case_data.get('solution', '')}"
        ...
        if "aviation_knowledge_base" in collections:
            # 使用时间戳+ID作为唯一ID
            point_id = int(time.time() * 1000000) + case_id
            ...
            client.upsert(
                ...
                points=[PointStruct(
                    ...
                    payload={
                        ...
                        "description": case_data.get("fault_symptom", ""),
                        "problem": case_data.get("fault_symptom", ""),
...
def import_cases(data_file: str = "repair_cases.json", dry_run: bool = False):
    """批量导入维修案例"""
    print("=" * 60)
    print("批量导入维修案例")
    print("=" * 60)
    ...
    print(f"找到 {len(cases)} 条维修案例")
    
    if dry_run:
        print("\n[试运行模式] 以下案例将被导入：")
        for i, case in enumerate(cases, 1):
            print(f"  {i}. {case.get('title', '无标题')} - {case.get('device_type', '未知设备')}")
        return
...
def main():
    parser = argparse.ArgumentParser(description="批量导入维修案例")
    parser.add_argument("--file", default="repair_cases.json", help="数据文件名（默认: repair_cases.json）")
